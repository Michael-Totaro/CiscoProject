...
def parse_command(device, command):
    ...
    print(f"Running parse_command command {command} on device {device.name}")
    try:
...
def get_device_inventory(device, show_version, show_inventory):
    ...
    if device.os in ["ios", "iosxe"]:
        ...
        print(show_inventory[device.name]["output"]["module_name"].values())
...
if __name__ == "__main__":
    ...
    network_inventory = []
    for device in testbed.devices:
        network_inventory.append(get_device_inventory(testbed.devices[device], show_version, show_inventory))
